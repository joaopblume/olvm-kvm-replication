...
dag = DAG(
    'atualiza_certificado_dr_target',
    default_args=default_args,
    description='Atualiza certificados CA do OLVM DR (Target)',
    schedule=None,  # Manual
    catchup=False,
    max_active_runs=1,
    tags=['certificado', 'target', 'dr']
)
...
def validate_environment(**context):
    ...
    # Carregar configurações TARGET
    target_ovirt_url = Variable.get('target_ovirt_url')
    cert_path = Variable.get('target_ovirt_cert_path')
    
    # Validar URL
    if not target_ovirt_url or 'http' not in target_ovirt_url:
        raise ValueError(f'URL do OLVM DR inválida: {target_ovirt_url}')
    ...
    # Construir URL do CA
    ca_url = get_ca_url_from_ovirt_url(target_ovirt_url)
    
    logging.info(f'OLVM DR URL: {target_ovirt_url}')
...
def backup_current_certificate(**context):
    ...
    if os.path.exists(cert_path):
        ...
        logging.info(f'Backup DR criado: {backup_path}')
        ...
        logging.info('Certificado DR atual não existe, não será feito backup')
        return {
            **cert_info,
            'backup_created': None
        }
...
def download_ca_certificate(**context):
    """Baixa o certificado CA do OLVM DR"""
    ...
    try:
        logging.info(f'Baixando certificado DR de: {ca_url}')
        ...
        if os.path.getsize(tmp_path) == 0:
            raise ValueError('Certificado DR baixado está vazio')
        ...
        logging.info(f'Certificado DR baixado com sucesso: {os.path.getsize(tmp_path)} bytes')
        ...
        if os.path.exists(tmp_path):
            os.unlink(tmp_path)
        raise Exception(f'Falha ao baixar certificado DR: {str(e)}')
...
def install_new_certificate(**context):
    ...
    try:
        ...
        if not os.path.exists(final_cert_path):
            raise Exception('Falha ao instalar certificado DR')
        
        final_size = os.path.getsize(final_cert_path)
        logging.info(f'Certificado DR instalado com sucesso: {final_cert_path} ({final_size} bytes)')
        ...
        if backup_path and os.path.exists(backup_path):
            logging.error(f'Erro na instalação DR, restaurando backup: {backup_path}')
        ...
        raise Exception(f'Falha ao instalar certificado DR: {str(e)}')
...
def test_new_certificate(**context):
    ...
    target_url = cert_info['target_url']
    cert_path = cert_info['cert_path']
    
    try:
        # Fazer uma requisição de teste usando o novo certificado
        test_url = f"{target_url.replace('/ovirt-engine/api', '')}/ovirt-engine/api"
        ...
        if response.status_code in [200, 401]:  # 401 é OK (sem autenticação)
            logging.info('Teste do certificado DR: ✓ SUCESSO')
            return {
                **cert_info,
                'test_result': 'success',
            ...
            raise Exception(f'Teste DR falhou: HTTP {response.status_code}')
            
    except requests.exceptions.SSLError as e:
        raise Exception(f'Erro SSL no teste do certificado DR: {str(e)}')
    except Exception as e:
        raise Exception(f'Falha no teste do certificado DR: {str(e)}')
...
def cleanup_old_backups(**context):
    ...
    if len(backups) > 5:
        for backup_path, _ in backups[5:]:
            ...
            logging.info(f'Backup DR antigo removido: {backup_path}')
    
    logging.info(f'Limpeza DR concluída. {len(backups[:5])} backups mantidos.')
